...
function check_result() {
    ...
    fetch('results.php', requestOptions).then(res => res.json()).then(response => {

        console.log(response)
        let result = document.querySelector('#result');
        result.classList.remove('not-show');
        result.classList.add('quiz-result');
      /*   result.getElementsByTagName('h1')[0].innerText = response.NAME; */
        result.getElementsByTagName('h3')[0].innerText = response.TITLE;
        result.getElementsByTagName('p')[0].innerText = response.CONTENTS;
        result.getElementsByTagName('p')[1].innerText = "Gli utenti hanno ottenuto questo risultato: ".concat( parseInt(response.NUM_INTERACTIONS) > 1 ? `${response.NUM_INTERACTIONS} volte` : `${response.NUM_INTERACTIONS} volta`);
